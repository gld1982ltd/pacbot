...
var cacheFiles = function (files) {
    ...
    return _.chain(files).map(function (f) {
        return fss.target(f);
    }).map(function (f) {
        if (fss.directoryExists(f)) return fss.allTargetFilePaths(f);
        return f;
    }).flatten().uniq().map(function(f) {
        var mime   = filter.get('mime', fss.filetype(f))();
        var prefix = config.root ? config.root : '';
        var suffix = config.timestamp && mime ? '?v=' + config.time : '';
        return path.join(prefix, fss.relative(f)) + suffix;
    }).value();
};
